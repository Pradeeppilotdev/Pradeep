...
const ParticleText = ({
  ...
  useEffect(() => {
    ...
    const sampleText = async () => {
      ...
      for (let y = 0; y < offscreen.height; y += step) {
        for (let x = 0; x < offscreen.width; x += step) {
          ...
          if (alpha > 40) {
            const textX = align === 'left' ? 0 : width / 2 - offscreen.width / 2
            targets.push({
              x: textX + x,
      ...
      const maxParticles = Math.max(700, Math.min(2600, Math.floor((width * height) / 150)))
    ...
    const intersectionObserver = new IntersectionObserver(handleVisibility, { threshold: 0 })
